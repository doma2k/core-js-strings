...
function countVowels(str) {
  ...
  const xyz = str.match(regex);
  return xyz ? xyz.length : 0;
}
...
function isPalindrome(str) {
  const proc = str.toLowerCase().replace(/[\W_]/g, '');
  const rev = proc.split('').reverse().join('');
  return rev === proc;
}
...
function extractNameFromTemplate(value) {
  ...
  return `${name[1]} ${name[2].slice(0, -1)}`;
}
